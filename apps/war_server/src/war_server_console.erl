...
remove([NodeStr]) ->
    ...
    case net_adm:ping(Node) of
        pang ->
            throw({error, not_reachable});
        pong ->
            war_consensus:rcfg_remove(Node)
    end.
...
system_check() ->
    case war_server:ping() of
        pong ->
            ok;
        _ ->
            throw({error, server_unavailable})
    end,
    case war_server:ping_backend() of
        pong ->
            ok;
        _ ->
            throw({error, backend_unavailable})
    end,
    case war_server:ping_service() of
        pong ->
            ok;
        _ ->
