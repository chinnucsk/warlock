...
handle_cast({adopted, {CurrBallot, PValues}},
    ...
    war_consensus_rcfg:set_master(CurrBallot),
...
handle_cast({master_adopted, CurrBallot, OldMaster},
    ...
    NewMonitors = case {OldMaster /= ?SELF_NODE, OldMaster /= undefined} of
        {true, true} ->
            ...
            %war_consensus_rcfg:node_down(OldMaster),
            monitor_members();
        {true, false} ->
            monitor_members();
        {false, _} ->
...
handle_cast({preempted, ABallot}, #state{ballot_num=CurrBallot,
    ...
    {NewBallot, NewTimerRef} =
        case war_consensus_util:ballot_greateq(ABallot, CurrBallot) of
            true ->
                NextBallot = war_consensus_util:incr_ballot(CurrBallot, ABallot),
...
handle_cast(incr_view, #state{hash_table=HT,
    ...
    NewBallot = war_consensus_util:incr_view(Ballot),
...
handle_info({timeout, _Ref, renew_master}, #state{ballot_num=Ballot}=State) ->
    ...
    NewState = case war_consensus_state:is_master() of
        true ->
            war_consensus_rcfg:set_master(Ballot),
            State;
        false ->
...
handle_info({'DOWN', _MonitorRef, process, {?LEADER, Node}, Info},
    ...
    case war_consensus_state:get_node_status(Node) of
        undefined ->
            ok;
        _Status ->
            war_consensus_rcfg:node_down(Node)
    end,
...
spawn_commander(PValue) ->
    war_consensus_commander:start({?SELF, PValue}).
    %war_consensus_commander_sup:create({?SELF, PValue}).

spawn_scout(Ballot) ->
    war_consensus_scout:start({?SELF, Ballot}).
...
check_master_start_scout(#state{ballot_num=Ballot,
                                timer_ref=OldTimerRef}=State) ->
    NewTimerRef = case war_consensus_state:is_status(valid) of
        true ->
            % If lease is going to timeout and we are not the master
            % then start scout
            LeaseTime = war_consensus_state:get_lease_validity(),
            case {(LeaseTime =< ?MIN_LEASE), war_consensus_state:is_master()} of
                {true, false} ->
...
check_master_start_commander(NewPValue) ->
    ...
    case war_consensus_state:is_master() andalso (LeaseTime > ?MIN_LEASE) of
        true ->
...
monitor_members() ->
    Members = war_consensus_state:get_members() -- [?SELF_NODE],
