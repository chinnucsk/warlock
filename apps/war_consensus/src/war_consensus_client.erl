...
exec(#rop{client=undefined}=ROp) ->
    war_consensus_rcfg:callback(ROp);
exec(#rop{}=ROp) ->
    {Client, TRef} = ROp#rop.client,

    % Execute rcfg on all nodes
    Result = war_consensus_rcfg:callback(ROp),
...
-spec init([]) -> {ok, #state{}}.
init([]) ->
    {ok, #state{requests=war_util_ets_ht:new()}}.
...
handle_call({sync_propose_rcfg, Operation}, From, #state{requests=Requests}=State) ->
    ...
    war_util_ets_ht:set(TRef, From, Requests),
...
handle_cast({response, TRef, Result}, #state{requests=Requests} = State) ->
    ...
    case war_util_ets_ht:get(TRef, Requests) of
        not_found ->
            ?LINFO("CONC::Error, req not found");
        Caller ->
            gen_server:reply(Caller, Result),
            erlang:cancel_timer(TRef),
            war_util_ets_ht:del(TRef, Requests)
    end,
    {noreply, State};
