...
remove(Node) ->
    ...
    case war_consensus_state:get_master() =:= [Node] of
        true ->
            {error, master_removal};
        false ->
            war_consensus_client:sync_propose_rcfg(remove_op(Node))
    end.
...
transfer_master(Node) ->
    case war_consensus_state:get_master() of
        [Node] ->
            {error, cannot_transfer_to_master};
        _ ->
            war_consensus_client:sync_propose_rcfg(transfer_master_op(Node))
    end.
...
callback(#rop{type=election,
    ...
    war_consensus_state:set_master(Node, Lease),
...
callback(#rop{type=join,
    ...
    case war_consensus_state:is_master() of
        true ->
            rpc:call(Node,
                     war_consensus_rcfg, cluster_add_node, [Node, ClusterDelta]),
...
callback(#rop{type=repl_join,
    ...
    IsMaster = war_consensus_state:is_master(),
...
callback(#rop{type=node_down, data=Node}) ->
    war_consensus_state:set_node_status(Node, down);
callback(#rop{type=remove, data={Node, ClusterDelta}}) ->
    IsMaster = war_consensus_state:is_master(),

    % Remove the given node from state
    war_consensus_state:remove_node(Node),
    war_consensus_state:set_cluster_delta(ClusterDelta),
...
cluster_add_node(Node, ClusterDelta) ->
    war_consensus_state:set_node_status(Node, valid),
    war_consensus_state:set_cluster_delta(ClusterDelta).
...
election_op(Ballot) ->
    #rop{type=election,
         data={?SELF_NODE, war_consensus_util:get_lease(), Ballot}
        }.
...
sync_with_cluster(SeedNode) ->
    [Master] = rpc:call(SeedNode, war_consensus_state, get_master, []),
    SysState = rpc:call(Master, war_consensus_state, get_state, []),
    NewSysState = clean_state(SysState, [node, status]),
    war_consensus_state:set_state(NewSysState).

% Update local node status to make sure it does run master election
disable_local_leader() ->
    war_consensus_state:set_node_status(join),
...
add_member(Type, Node, ClusterDelta, Callback) ->
    ...
    war_consensus_client:propose_rcfg(Operation).
...
get_sync_member() ->
    AllMembers = war_consensus_state:get_members(),
    Master = war_consensus_state:get_master(),
    case AllMembers of
        Master ->
            hd(Master);
        _ ->
